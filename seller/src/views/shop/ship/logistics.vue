  ...
  <div class="logistics">
    ...
    <Modal v-model="openModal" :title="openModalTitle" @on-ok="submit" @on-cancel="cancelModal">
      ...
      <Card v-if="onpenText" class="modalStyle">
        <h3 style="color: #ff3c2a; margin-bottom: 10px">Please enter 详细信息</h3>
...
export default {
  ...
  data() {
    return {
      ...
      openModalTitle: 'Opening信息',
      ruleValidate: {
        // customerName: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        // payType: [{ required: true, message: "Please enter 必填项" ,trigger: "change" }],
        // expType: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        // customerPwd: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        // monthCode: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        // sendSite: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        // sendStaff: [{ required: true, message: "Please enter 必填项" ,trigger: "blur" }],
        ...
        },
        {
          ...
          render: (h, params) => {
            if (!params.row.selected) {
              return h("div", [h("tag", {props: {color: "volcano"}}, "Close")]);
            } else {
              return h("div", [h("tag", {props: {color: "green"}}, "Opening")]);
            }
          }
        },
        {
  ...
  methods: {
    ...
    frontDownload(val) {
      ...
      //根据点击按钮来下载不同文件
      if (val === 'use') {
        a.href = "static/instructions.xlsx"; // 给a标签的href属性值加上address，注意，这里是绝对路径，不用加 点.
        a.download = "使用说明.xlsx"; //设置下载文件文件名，这里加上.xlsx指定文件类型，pdf文件就指定.fpd即可
      } else if (val === 'type') {
        a.href = "static/logisticsType.xlsx"; // 给a标签的href属性值加上address，注意，这里是绝对路径，不用加 点.
        a.download = "快递类型.xlsx"; //设置下载文件文件名，这里加上.xlsx指定文件类型，pdf文件就指定.fpd即可
      }
      a.style.display = "none"; // 障眼法藏起来a标签
      document.body.appendChild(a); // 将a标签追加到文档对象中
      a.click(); // 模拟点击了a标签，会触发a标签的href的读取，浏览器就会自动下载了
      a.remove(); // 一次性的，用完就deletea标签
    },
    ...
    submit() {
      if ( !this.row.selected) {
        API_Shop.logisticsChecked(
          ...
          this.$Modal.remove();
          if (res.success) {
            this.$Message.success("Logistics company Openingsuccess");
            this.init();
          }
        });
      } else {
    ...
    close(v) {
      this.$Modal.confirm({
        title: "确认Close",
        content: "您确认Close此Logistics company ?",
        loading: true,
        onOk: () => {
          API_Shop.logisticsUnChecked(v.logisticsId).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("Logistics company Closesuccess");
              this.init();
            }
          });
        }
